...
$$
Survie ~~ accelerée \Longrightarrow ~ Hasards ~ ~proportionnels
$$
...
##################################################################
########## densité de weibull + plots ############# ignored ######
##################################################################
...
text(60,0.05, expression(alpha > 1))
text(20,0.01, expression(alpha<1))
text(50,0.02,expression(alpha==1))
curve(weibHaz(x, shape=0.5, scale=1/0.03), add = TRUE, col = "green")
curve(weibHaz(x, shape=1, scale=1/0.03), add = TRUE, col = "blue")
```

```{r,fig.align='center',fig.out="50%", include = F}
# Graphic
knitr::include_graphics("weib_haz.png")
# End graphic 
```
...
Ceci introduit donc une covariable qu'on a nommé *Régime*, ses valeurs sont : $ \{Democracy, ~ Non- Democracy ~  \}$.  
...
knitr::kable(sample_dem,align = 'c', caption = "This is the table caption")
```
...
legend(x="topleft",
       legend = c("Régimes démocratiques","Régimes non-démocratiques","Survie log-normale"),
       lty =c(1,2,1,2,1,2),
...
```{r, echo = FALSE, fig.align='center', out.width= "33%",fig.cap = "Estimations paramétriques des lois Log-normale et Gompertz. Les courbes en violet et en doré représentent l'estimation non paramétrique de la fonction de survie par Kaplan-Meier. Les rouge designe la fonction de survie estimée des lois Log-normale (à gauche) et Gompertz (à droite) à l'aide de survreg"}
