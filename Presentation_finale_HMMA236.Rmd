...
sample <- sample %>% select(-un_region_name, -leaderspellreg, -un_continent_name)
sample <- rename(sample, Gouv = ehead, Pays = ctryname, Démocratie = democracy, Régime = regime, "Date d'entrée" = start_year, Durée = duration, Statut = observed)
...
\newpage
```{r, include = F}
democracy.weib <- flexsurvreg(Surv(duration, observed) ~ regime, data = df, dist = 'weibull' )
...
ICH95 <- cox$coefficients +sqrt(diag(cox$var))*1.96 # estimation cox =0.176 [-0.38 ; 0.74] 
AppartenanceIC <- (coefficients_weibull >= ICL95 || coefficients_weibull <= ICH95)
D <- data.frame(row.names = covariables, coefficients_weibull, cox$coefficients, ICL95, ICH95, AppartenanceIC)
knitr::kable(D, align = 'c')
```
On voit que les estimations sont assez différentes entre les coefficients estimés avec le modèle log-linéaire de weibull et les coefficients estimés dans le modèle de Cox. 

Cependant, l'imprécision de l'estimation (écarts-type estimés sont environ égaux à 0.22 pour les coefficients de modèle de Cox) ne permet pas de conclure à une différence significative.
