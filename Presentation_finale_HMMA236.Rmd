...
nous obtenons des $p$-values significatives sur toutes les modalités, ce qui signifie que aucun des coefficients n'est nul.
...
On remarque que les coefficients sur le modèle de cox et des autres modèles paramétriques sont proches au centième près. On peut aussi ajouter que leur signe sont opposés (log?).
De plus dans le modèle de cox les écart-types y sont un peu plus grand environ +0.3 et évidemment nous n'avons pas l'estimation des paramètres.


cox
$$
h(t|Z)=h_0(t). exp(\theta^tZ)
$$
survie accélérée
$$
h(t|Z)=h_0(exp(\theta^tZ)). exp(\theta^tZ)
$$
