...
La première prorpriété indique la log-linéarité de la fonction de hasard cumulé. Et donc, couplée à l'estimateur de la survie de Kaplan Meier dont le calcul est possible grâce au données, elle permet l'évaluation graphique de la validité du modèle.
...
\subsubsection{Démocraties et non-démocraties}
...
Ceci introduit donc une covariable qu'on a nommée *Démocratie*,
...
Le modèle Log-normal est celui dont le critère AIC est le plus petit . C'est sur ce modèle qu'on devrait baser notre étude. Cependant, une évaluation graphique des différents modèles nous suggère que la loi de Gompertz semble plus appropriée pour l'estimation des données de survie comme on peut voir sur les graphiques ci-dessous.
...
\paragraph{Modèle Log-normal avec covariable}
\paragraph{Modèle de Weibull avec covariables}
...
AppartenanceIC <- (coefficients_lnorm >= ICL95 || coefficients_lnorm <= ICH95)
