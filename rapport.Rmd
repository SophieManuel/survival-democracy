...
\paragraph{Modèle Log-normal avec covariables}
...
ICL95 <- coefficients_weibull -sqrt(diag(modweib$var)[-1 : -6])*1.96 # bornes de l'IC 95% des coeff.
ICU95 <- coefficients_weibull -sqrt(diag(modweib$var)[-1 : -6])*1.96 # estimation cox =0.176 [-0.38 ; 0.74] 
AppartenanceIC <- (cox$coefficients >= ICL95 || cox$coefficients <= ICU95)
...
ICL95 <- coefficients_lognorm -sqrt(diag(modweib$var)[-1 : -6])*1.96 # bornes de l'IC 95% des coeff.
ICU95 <- coefficients_lognorm -sqrt(diag(modlnorm$var)[-1 : -6])*1.96
...
<https://github.com/SophieManuel/survival-democracy>
